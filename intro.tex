...
\textbf{Play with or without magic.} The magic system is designed in such a way that it does not contradict any laws of physics. Magic is conceived merely as telepathic control over natural phenomena. If the physical ingredients are not present, magic is not possible. Magic does not create matter or energy. This design makes it possible to have a game without magic using the same system. A game without it is complete. 
...
In ability, spell and offensive actions design, the game uses a triangle of 3 characteristics of which an action can only have 2 items: cheap to use (cost in time, stamina and resources), high impact (high damage, strong effects), and hard to avoid (costs a lot of resources to avoid or has high chance of hitting). This carves specific tactical niches and makes sure that no single action will be spammable to win.
...
\textbf{High impact demands high commitment.} This is a lethal game, but also a strategic one. Victory and defeat are designed to be a consequence of the player's decisions. The game provides many tools to avoid the loss of a character, all of which involve player choice. The goal is to always make players feel that what happens to their character is a consequence of their decisions, never an arbitrary happenstance. 

\textbf{Fast to resolve.} This applies especially to combat. The math is meant to be very expressive of the real world, but also as quick as possible to calculate. It is a compromise solution tuned to get the most depth per time spent. The game is designed to be played with pen and paper, and the math is meant to be done in the head. Online tools are available and do speed up the process, but they are not necessary.

The game speeds up by a lot with mastery. Some of the mechanics seem to have several steps, but they can be collapsed to a single simple operation in many scenarios. In the ones that don't, the action really counts for the outcome of the scene. Taking a bit of extra time in a mechanic while dwelling in success is much more pleasant than calculating to find out nothing happens. The game takes some liberty to scale cognitive load with consequence magnitude.
