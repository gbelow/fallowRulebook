...
\textbf{Healing:} Each IL recovered increases hunger and thirst by 1. Healing is not possible if hunger values are at their maximum value. 

\textbf{Healing Wounds:} require medicine or magic to heal and does not increase hunger or thirst.
...
\textbf{Sleep } It is an action that costs 2 exploration turns. It heals IL, recovers exhaustion, and can only be done once per day. The character is unconscious during sleep. After sleeping, the exhaustion value is $exhaustion/2 -exposure$. The character heals IL equal to $3+(health-exposure)/2$, where health is the health skill and exposure represents the absence of mental and physical stress.

\textbf{Short Rest:} The character has the option to spend an ET doing nothing, which recovers 1 exhaustion. Every two short rests in a row cost 1 hunger, 1 thirst and heal 1 IL (which also increase hunger and thirst). Any time a character does not participate in an exploration action, they are doing nothing and short rest automatically.

\textbf{Exposure:} Each terrain has a value that represents how rough it is to spend time there. This represents how physically and mentally stressful the situation is. Generally, the value ranges from 5 to 0 for places that resting is possible. A 0 means you are as comfortable as sleeping in your comfortable home's bed. The character can lower exposure by using items.

If $3+(health-exposure)/2$ is lower than 0, it is impossible to rest or sleep.
...
\textbf{Long Camping:} Sometimes characters need to rest for several days to heal up. In that case, consider that any shelter that was built and any campfires that were made last for several days stay as they are. The exposure value stays constant. Every day, characters heal $5+(health-exposure)/2$ and gain +2 hunger and +2 thirst. They can continue doing this until water or food runs out. Healing abilities can be used for 1 ET per day per healer. Exhaustion should be zero by the second day for everyone.

%-----------------------------------------------------------------
\section{Exploration}

\textbf{Exploration Action:} are \textbf{Traveling}, \textbf{Searching}, \textbf{Preparing}. Multiple characters can participate in a single exploration action, but whenever the action requires a test, only one character rolls for it.

\subsection{Wear}

\textbf{What cause wear:} Any exploration action causes Wear. The effect is applied to everyone who participated in the exploration action, independent of them rolling for it or not.
...
Traveling at night significantly reduces the team's visibility range, making exploration more difficult. If the group has a light source or the ability to see in the dark, the DL for travel and search is increased by just +5. If the group is without vision, search and travel increases by +10.

The day can have any number of periods of day and night. This depends on the season and the location on the map. The standard is 4 periods of day and 2 of night.
...
\textbf{Traveling distance:} from one node to the next can cost multiple ETs. There is no need for placeholder nodes in the middle of two important ones just to conform to the duration of 1 ET. 
...
\textbf{Knowledge enablers:} Many events require a knowledge as enabler, which can be fulfilled by any member of the team that is present in the exploration action.
