...
\begin{itemize}
    ...
    \setlength{\parskip}{0em}
    \item 15 = Dead
    \item 12 = Unconscious + Dehydrated
...
\subsection{Rest and exhaustion}
...
\textbf{Healing Wounds:} all wounds heal by half the value of IL recovered. If the IL is already zero, but there are still wounds, continue healing the wound at the same rate, but do not increase hunger nor thirst.

\textbf{Short Rest:} The character has the option to spend an ET doing nothing, which recovers 1 exhaustion +1 IL. Every two short rests in a row cost 1 hunger and 1 thirst. Any time a character does not participate in an exploration action, they are doing nothing and short rest automatically.

\textbf{Exposure:} Each terrain has a value that represents how rough it is to spend time there. This represents how physically and mentally stressful the situation is. Generally, the value ranges from 5 to -2. Anything higher than 5 prevents resting, while -2 means you are as comfortable as sleeping in your home's bed. More explanation on the Terrain subsection.

\subsection{Wear}

Performing any exploration action that is not resting causes wear. The effect is applied to everyone who participated, independent of them rolling for it or not. If they were in the group that executed the exploration action, they receive the respective wear.

The standard wear is 1 IL + 1 exhaustion per ET, and it is applied at the end of the ET. It is possible for the exploration action to have increased wear depending on the terrain. This is explained in the terrain  subsection.
...
\skill{Exploration}{ Knowledge + Awareness }{ This skill is used to perform Travel and Search actions. Its value is equal to the specific knowledge about the location + the detection proficiency. 

\textbf{General Knowledge:} in the absence of specific knowledge of the region, use Awareness + $knowledge/2$, where knowledge is one of the general knowledges. Use survival for searching and tracking in the wilderness, geopolitics for traveling, and architecture for tunnels and large structures. The general knowledge is not as good as specific, but it helps.

\textbf{Acquiring Specific knowledge:} Acquiring maps, reaching landmarks, and asking for information improve local knowledge. These are events unlocked through traveling or in preparation for travel. 
...
\textbf{Tracking} is done with an exploration test but requires a specific ability. When tracking someone, make an exploration test against the terrain's DL for tracking. Adjust the DL based on how old the trail is, how many creatures there were, and their size and what they were carrying. A crit or hit means that the quarry was found if it is in the same node, or which direction they went to if not. 

\textbf{Grazes and misses:} If characters dont find any events with search nor the path they were looking for with travel, nor their quarry when tracking, they find a random event in the node or go to another random node. The explanation is in the events subsection.
...
\textbf{Campfire:} The campfire allows cooking, provides a source of light and reduces exposure to cold and insects by 1. It requires firewood and some method to light the fire. \textbf{Searching for firewood:} requires characters to perform an exploration action to search for dry firewood. On a crit, they do not spend the exploration action and succeed. On a hit, they succeed. On a graze, they can either give up or spend double the time. On a failure, the action fails. The firewood lasts for a day.\textbf{Lighting the fire:} is done without a test if some items or spells are present, otherwise, try to use natural materials. Crits perform it in minutes, hits spend an ET, grazes gives the option to spend 2 ETs to succeed or lose one and misses just waste an ET.

\textbf{Cooking:} This test is made against the DL of the food and can require equipment to be done, like a campfire and a pot. On a critical success, all food is utilized. On a success, only 75\%. On a graze, 50\%, and on a miss, 25\%.
...
\textbf{The exploration loop} is designed to require preparation. Having tools to make a campfire, carrying tents and bedrolls, bringing a pot to cook, bringing food and water, and hiring a guide are all measures that should be thought of before starting an expedition. Exploration is a demanding task and can by itself cause as much harm as a battle.

\textbf{Exploration day Breakdown:} full day in which characters sleep once and explore 4 times is meant to net a negative IL sum. This is equivalent to pushing the pace.

A day where characters explore 3 times, rest once and sleep once is meant to net around 0 IL. This is a good traveling pace.
...
\textbf{Natural Shelter DL:} Standard difficulty to build a natural shelter.
...
\textbf{Wear:} Terrain can have value for extra wear. For example, spending an ET in the desert without shelter always inflicts an extra thirst. Moving through mountains may inflict extra exhaustion. 

\textbf{Terrain tags:}
\begin{itemize}
    \item wood: allows looking for firewood
