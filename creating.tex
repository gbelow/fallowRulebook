...
Stealth & Awareness & Dexterity  & yes \\

Cunning & Awareness & Intellect  & yes \\
Detection & Awareness+Sense & Intellect  & no \\
Exploration & Awareness & Intellect  & knowledge \\
...
\textbf{Awareness (INT)}: The character's ability to pay attention to what matters and not get distracted. Uses INT as talent.\\
...
\textbf{Attributes as skills:} sometimes, an attribute can be used as a skill in case no skills fit an action. For example, strength for lifting weight, agility to outrunning someone and stamina for holding breath. In that case, use attribute -10 as the skill value.
...
\textbf{ Natural Toughness (TGH)} define how much damage a character can withstand before receiving injuries. Their default value is 0.5xSTR xDM.  This can be interpreted as a larger body, tougher flesh or a stocky build.

% \textbf{Natural Poise} is a value that is used to detemine the character's resistance to being interrupted or stunned. It's basic value is equal to the character's STR x DM.
...
\textbf{Damage Multiplier (DM):} multiplies all values derived from STR of a character and their appropriately scaled gear. It's TGH and Poise, armor's defenses, weapon damage, abilities, and spells.
...
\setlength\itemsep{0em}
\setlength{\parskip}{0em}
\item Increases one talent by 1 at creation.
...
Whenever a player claims that the character knows something, the GM can ask for a knowledge check to determine if the character really knows it.
...
\textbf{Why have this?} This is a reward and a history tracker for everything the character knows about the world and their own story. 
...
\textbf{Animal handling - animal:} Knowledge about taming and riding a specific animal or type of animal. Some animals may not be rideable without this. \\
...
Spirit is the main talent for Sorcery. It is also used to determine the character's power of Will.
...
\textbf{Violent:} is compelled to use their fight or reaction surge to inflict damage on someone.

\textbf{Tanatosis:} Falls to the ground paralyzed for 1 turn and rests 2 times.

\textbf{Abusive:} is compelled to use their reaction surge to insult allies, increasing the test by +1 for the team.

\textbf{Traitor:} is compelled to use their reaction surge to negotiate with the opponent. If the target is unable to communicate, they run for 1 turn.

\textbf{Martyr:} is compelled to use their reaction surge to protect an ally.
