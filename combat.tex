...
\item \textbf{Tired/Exhausted/Confused} Penalty of -1 mental/-2 mental/-4 mental. Becomes vulnerable to domination attacks. 
...
\skill{Stealth}{ Awareness }{ Skill to remain unnoticed. It involvels stepping lightly, moving with appropriate timing, keeping to the shadow, avoiding making noise. 

\textbf{Stealth events:} are a description given by the player of what exactly the character is trying to do with their stealth, which defines which senses the detectors are using and sets a bonus or penalty to the stealth test. The usual values for fair propositions should stay between -5 and +5. The value of this test is used as a DL against the detection of every candidate detector.
...
\textbf{Stealth test:} During a stealth event, the player makes a stealth test against Detection to stay undetected. On hits or crits, the character stays unnoticed. On grazes, the character is detected and can be reacted against. On misses, he character is detected and, if in combat, the opponent may take their turn immediately.
...
\skill{Detection}{2x Awareness}{ Detection is used to detect things in stealth or just for looking for things in general.

\textbf{Detected and Undetected status} To be considered undetected, a character must be out of the field of detection of all active senses of another character at the beginning of a round. Each character has their own list of detected characters. An undetected character cannot be reacted against.
...
\textbf{Senses:} Any detection event will target a specific sense. The basic usable senses are vision, hearing, smell and touch. In each detection attempt, a sense must be chosen to determine how the detection happens. Senses can be active or passive. 
...
The dificulty to detect or perform stealth depend on which sense is the dominant one for that detection event. 
...
\begin{itemize}
    ...
    \item same room (within about 10m), no penalty
