...
\begin{proplist}
    \item \textbf{Armor Bypass:} This costs a number of HOP defined by the targets armor deflection. It hits the inner layer of the armor, if the armor has one. Only attacks from weapons with the precise property can use this effect.
    \item \textbf{Penetrating:} Costs a number of HOPs equal to the target's deflection to be able to use cutting damage against objects with the same hardness. Behaves like extra cut against fiber armor and after already penetrating same hardness objects. This can only be done by weapons with the penetrating property. 
    \item \textbf{ Extra cut:} Increases cutting damage by 1xDM per HOP. Only weapons with the bladed property can use this. Increases by 2xDM per HOP for bladed and piercing weapons.
...
\begin{proplist}
    ...
    \item \textbf{Head:} -5 to the attack test. Getting a Tier 3 injury from blunt, cutting or electrical damage or stun to the head causes unconsciousness that lasts at least 1 minute without intervention. Tier 4 damage causes instant death. Armor bypass at the head always hits flesh.
\end{proplist}
...
\textbf{Heavy Attack:} It is possible to make a heavy attack with weapons that have the "heavy" property. Heavy I increases damage of physical attacks by +0.5xSTR, but have an added cost of +1AP. Heavy II adds +1xSTR, costs +2AP+1STA, and gives a -2 penalty to hit. Heavy III adds +1.5xSTR, costs +3AP+1STA, and gives a -3 penalty to hit. Bonus applies to blunt damage and cutting damage.
