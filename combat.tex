...
\textbf{Morale test:} At the beginning of each round, all characters make a will test against the terror of the moment. A graze leaves the character frightened, and a failure activates the character's limit stress action.

DL = -2 + aggravating factors

Aggravating factors: \\ 

\begin{itemize}
    \item Sensory deprivation(being oblivious): +3
    \item Reduced sensory quality(being disoriented): +2
    \item Seriously injured ( IL > 10): +3
    \item In melee range: +2
    \item Being on fire or suffocating: +5
    \item Out of STA: +2
    \item Monstrous presence: variable. Depends on presence.
\end{itemize}

\textbf{Intimidation:} A character can make an intimidation attempt as part of an action in combat. This increases the value of the DL of the next morale test by the character's charisma. The intimidating action costs 2 AP and can be made by anyone in the team after causing a T1 injury or higher to an enemy. 

\textbf{Taunt:} In combat, a taunt is an attempt to make the target infuriated. It costs 2 AP and must be made immediately after someone in the team is missed by a direct attack or receiving zero damage from one. Add charisma to the morale test of the opposing side in the next morale check.

Multiple intimidations and taunts don't stack. Only the highest charisma 
...
% \item \textbf{Charmed:} Behaves like everyone is a contact. Cannot coexist with afraid or enraged.
