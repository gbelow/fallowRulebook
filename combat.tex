...
\textbf{Preparation:} At the start of combat, the miniatures are positioned in their locations, and each character starts with 8 AP by default. 
...
\textbf{Action surge:} All characters are entitled to one action surge per round. There are 4 basic types of action surges: movement, combat, reaction and focus.

\begin{proplist}
    \item \textbf{Movement surge:} 3 STA to gain AGI/2 AP that must be used for movement immediately and allow running until the end of the turn.
    \item \textbf{Combat surge:} 3 STA to gain 4 AP that must be used for attacks or movement and must be used immediately.
    \item \textbf{Reaction surge:} 3 STA to gain 4 AP, but can only use reactions until the end of the round.
    \item \textbf{Focus surge:} Gains no AP, but this surge is required to use ranged attacks, casting spells and using standard actions.
\end{proplist}

\textbf{End of the round:} A round ends when no one has AP left and the remaining agree to end the round. When the round ends, all characters get reset to 8 AP minus any negative AP they had. Any unspent AP is lost.

\textbf{Rest:} is an action that costs 4 AP and recovers STA by an amount equal to STA/4. The character is allowed to move 4 AP worth of careful movement while resting during their own turn. The AP loss from ending a round without spending all AP can be prevented by resting, which can make AP negative, as long as it starts next round positive.
...
\textbf{Suffocation:} If the character cannot breathe at the beginning of a round, they lose 1 STA and cannot Rest. 

\textbf{Negative STA:} STA can become a negative number, but only as a result of suffocation. If STA is lower than -5, they drop unconscious. If it goes under -10, they die.
...
\item \textbf{getting up:} Removes the prone condition.
\item \textbf{jumping:} A jump has a vertical distance equal to half the horizontal distance. If performed during a run, increase the base calculation of the horizontal distance of a long jump to match that of running. Movement cannot be voluntarily interrupted in the middle of a jump. If performed backwards, the horizontal distance is halved. 
\item \textbf{crawling:} is the only usable movement speed while \textbf{prone}.
\item \textbf{swimming:} is the only usable movement speed while swimming. It also applies the \textbf{prone} condition.

\item \textbf{trample:} The character attempts to pass through an occupied space. If the opponent does not move out of the way, they get trampled, as explained in the force skill in melee combat.
...
The swimming test should not be redone more than once per round unless the swimming conditions change.

Characters attempting to swim with one hand receive a -3 penalty on the test.

A swimming test in a deep pool is considered a DL of -2. Swimming at a beach with waves is 0. Swimming in a swift river or choppy sea would be around 4. Swimming under a storm or in a raging river is around a 7.
}

\skill{Climbing}{AGI - 10}{ Make a climbing skill test to overcome a DL and climb a surface.
...
A character who is climbing is considered prone. An attack that interrupts the climber will require them to redo their climbing test with a -3 penalty. In the case of multiple climbing tests in a single turn, always take the worst result. Climbing with one hand incurs a -3 penalty.
...
\skill{Balance}{AGI - 10}{ Skill to perform acrobatics, avoid falling from losing balance, and mitigate damage from falling.

\textbf{Difficult terrain} Every time the character moves through difficult terrain, they must make a balance test. The DL is based on how slippery, unstable, long, and narrow the path is. The character decides the type of movement they wish to use, then makes the skill test. On a critical, all movement is inconsequential. On a success, only moving at a normal speed is safe, but not jumping or running. On a graze, moving at a careful speed is safe. On a miss, only crawling is allowed. If the character enters or moves in difficult terrain using a type of movement they did not manage well enough, they receive some consequence (usually falling).

\textbf{Fall skill test:} Falling requires a balance test to decide how well the landing or dive goes. On a critical on the balance test, reduce the damage by 50\% and prevent T0 and T1 injuries. On a hit, reduce damage by TGH. On a graze, takes full damage and falls prone, and on a miss takes full damage, falls prone and hits the head. Receiving a T1 injury or higher always makes the character prone and stuns.

\textbf{Choosing Fall DL:} When falling, the DL to fall properly is generally proportional to the fall height. Falls from small heights in a bad position are more difficult too. As a rule of thumb, DL is 1 per meter until a maximum of 15. As a common exception to the rule, DLs for 1m can be considered trivial at -5, then 0 for 2m.
...
\textbf{Injury level (IL):} Any time a character receives damage, they will increase their injury level. They will receive -1 as injury penalty for every multiple of 10. The penalty affects skills shown in the afflictions section.

\textbf{Injury Capacity:} is the maximum amount of injuries a character can take without collapsing and going immobile. Its value is 40.

\textbf{Death:} occurs when the character's injuries exceed 50 IL.
...
\textbf{Damage:} Any time a character receives damage, it must compare the amount received with its TGH for the damage type to determine the injuries caused. Armor increases the amount of damage that can be received without receiving injuries. The table shows how much IL they receive from injuries at each tier. 

\textbf{Damage Tiers:} 

\begin{stattable}{Damage Tiers}{@{}lccc@{}}
Name & Threshold & IL & Bleed \\
\midrule
Tier 0 & armor & 1 & 0 \\
\rowcolor{fallowtint}
Tier 1 & armor+TGH & 5 & 0 \\
Tier 2 & armor+2xTGH & 10 & 1 \\
\rowcolor{fallowtint}
Tier 3 & armor+3xTGH & 20 & 2 \\
Tier 4 & armor+4xTGH & 30 & 3 \\
\rowcolor{fallowtint}
Tier 5 & armor+5xTGH & 50 & 4 \\
\end{stattable}

\begin{dnote}
    \textbf{Interpreting IL:} A tier 0, or T0 injury means a small bruise or a patch of burned skin. A T1 injury is a deep cut that bleeds a little or a large bruise. A T2 is a cut down to the muscle or a potentially cracked bone. A T3 is definitely a cracked bone and internal organ damage with bleeding. A T4 is near system shock, with multiple broken bones, impalation through the chest and potentially limb amputation. A T5 means instant death.
\end{dnote}

\textbf{Types of damage:} are: blunt, cutting, burning, electric, radiant, and corrosive. Objects and creatures may have resistance or vulnerability to each of these types of damage. Different types of damage cause different kinds of effects. 

\textbf{Physical attacks:} have blunt and cutting damage components. If the weapon is not capable of cutting its target, the damage is blunt, otherwise, use the largest damage of the two, but apply the additional effects of both. 

\textbf{Blunt damage:} is defended by armor protection. Can cause interruption and stun.

\textbf{Cutting damage:} is defended by armor RES. Applies poison and electric damage directly, as if the target has no armor, when damage is at least T0.

\textbf{What cuts?} A weapon can cut something if their hardness is higher than their target's. There are 4 levels of material hardness. The first is for liquids. The second is for fabrics, flesh, and similar materials. The third is for wood, horns, bones and other solids. The fourth is for metals and rocks.

\textbf{Bleed:} is caused by blunt and cutting damage. For every STA spent and at the end of every round in combat, the character receives +1 to their IL per bleed intensity. When combat/chase ends, bleeding causes 3x its current value and stops by itself when untreated.

\textbf{Burn, radiant:} Their respective defense is INS. Dealing Tier 0 injury or higher leaves the target burning. 

\textbf{Corrosive:} Its respective defense is INS. Tiers 0 to I of damage leaves the target corroding at that tier of damage, which reapplies the damage until armor is removed. 

\textbf{Electric damage:} It is also defended by INS. It causes interruptions and stuns. This damage only deals half IL damage per tier.

\subsection{Additional effects:}

\textbf{Interruption:} is an effect that interrupts any action from its victim, making them lose its associated costs. Movement is cancelled, except running and jumping. It happens when a tier 1+ blunt or electric damage is inflicted or when the character is pushed.

\textbf{Stun:} An interrupt in which the target also loses 2 AP. It happens when blunt or electric damage causes Tier 3 injury or higher.

\textbf{Deflection:} is how hard it is to land additional effects on a hit. The natural deflection of a human body is +4. Any time a character is wearing armor, use the armor's deflection instead. 

\textbf{Success Overflow:} When a character receives a hit from an attack, they may receive additional effects, chosen by the attacker. HOPs are used to activate these effects. The effects are:

\begin{proplist}
    \item \textbf{Bypass:} This costs a number of HOP equal to the target's armor deflection and can only be done against rigid armor. It adds the value of 50\% RES or 50\% prot of the target armor to the damage, depending if the attack damage is blunt or cutting. Only attacks from weapons with the precise property can use this effect.
    \item \textbf{Bust:} Costs a number of HOPs equal to the target's deflection to be able to use cutting damage against objects with the same hardness. This can only be done by weapons with the penetrating property. 
    \item \textbf{ Slice:} Increases cutting damage by 1xDM per HOP. Only weapons with the bladed property can use this.
    \item \textbf{Smash:} Costs a number of HOPs equal to the target's deflection to add 2 xDM extra damage and upgrades an interrupt to a stun.
\end{proplist}


\subsection{Localized damage}


\textbf{Declaring a location:} must be done before performing any attack, otherwise the default applies. Each type of creature may have places that can specifically aimed at when using the attack. When someone declares that they are aiming at that specific point, the attack is modified according to the description of the location. 

To illustrate, here are some targets for humanoids. There are 4 locations: chest, hand, leg, and head.

\begin{proplist}
    \item \textbf{Chest:} Standard location, no penalty.
    \item \textbf{Hand:} -10 to the attack test. Attacks can switch target to the hand used for defense by spending 3 HOP when the target tries to block or intercept without a shield. Hands have no armor unless the character is wearing gauntlets. Maximum injury tier applied to the body is 2.
    \item \textbf{Leg:} 0 to the attack test. Maximum injury tier applied to the body is 3. 
    \item \textbf{Head:} -5 to the attack test. Getting stunned in the head causes unconsciousness that lasts at least 1 minute without intervention. Tier 4 damage causes instant death. Bypass at the head hits flesh, which ignores all armor.
\end{proplist}
...
\textbf{Wounds:} can be caused by receiving damage, as indicated in the table. A wound has a consequence that disables a character until healed. The amount of healing required is shown in IL wound on the table, and is tracked separately from the main IL. 

\begin{stattable}{Wounds}{@{}lccc@{}}
Wound & Cause & IL wound & consequence\\
\midrule
Broken hand & T2 blunt/cut & 10 & useless hand \\
Amputated hand & T4 blunt/cut & no heal & loses hand \\
Broken leg & T3 blunt/cut & 20 & lame\\
Amputated leg & T5 blunt/cut & no heal & loses leg\\
Shocked & T3 blunt+smash & 5 & immobile\\
\end{stattable}
...
\textbf{Injury (IL):} penalties affect STR, AGI, STA. This does not affect TGH. It affects all other usages of attributes.

\textbf{Sensory:} Strike, Defend, Accuracy, Reflex, Detection, Balance, Climb, Exploration, Stealth and Prestidigitation.

\textbf{Mental:} Penalties affect Exploration, Cunning, Will, Insight, persuasion, deception and all Knowledges and all spellcasting.  

The list of afflictions is:

\begin{proplist}
\item \textbf{Lame:} Cannot run and requires movement surge for basic movement. Takes +3 IL for traveling and Searching.
\item \textbf{Prone:} Can only crawl and stand up.
\item \textbf{Grappled:} Movement requires a test.
\item \textbf{Immobile:} Cannot move and cannot use any combat or movement skills other than escape. The DL to hit an immobile target is -5-1*SM. 
...
\item \textbf{Corroding 0/I} Applies a tier of injury every round. Can only be ended by specific items, jumping on water or removing the armor. Cannot be applied to unarmored.
...
\item \textbf{Afraid:} Receives -1 Mental and must make a morale test every beginning of round. The fear ends when the character succeeds on their Will test or becomes angry. A character cannot make a voluntary combat surge while afraid.
\item \textbf{Enraged:} Receives -1 Mental and must make a morale test every beginning of round. The anger ends when the character succeeds on their Will test or becomes afraid. A character cannot make a voluntary reaction surge while enraged.
...
\item \textbf{Confused:} Receives -3 Mental and must make a morale test every beginning of round. The confusion ends when the character succeeds on their Will test.
\item \textbf{Dominated:} Under the control of another character. Must obey commands for the duration but can make a Will test to break out of domination whenever a command constitutes a severe compulsion. Outside of combat, can make a test once per ET.

\item \textbf{Intoxicated I/II/III} -1/2/3 Mental.
\item \textbf{Tired/Exhausted} Penalty of -1 mental/-2 mental. Becomes vulnerable to domination attacks. It is possible to be confused without being tired or exhausted, but extreme exhaustion causes confusion.
\item \textbf{Weakened/Malnourished} -1/-2 Health. 
\item \textbf{Thirsty/Dehydrated} -1/-2 Health.
\item \textbf{Sick} -2 Health.
\end{proplist}
...
\skill{Strike}{Melee}{ Striking consists of a melee weapon attack on a target using the strike skill test against the opponent's defense. Grazes deal 50\% damage and misses do nothing. Hits deal full damage, and any extra points over a hit can be used to cause additional effects.

\textbf{Attack against equipment:} An attack can be made against a weapon, shield, or any object in order to break them. RES Rules on equipement breakage can be found in the equipment chapter.

Other variations of melee attacks are available depending on the type of weapon used. The type of attack must be declared when used. The variations can be combined, but braced and hook attack cannot be combined with anything.

\textbf{Heavy Attack:} It is possible to make a heavy attack with weapons that have the "heavy" property. Heavy I increases damage of physical attacks by +0.5 x STR xDM, but have an added cost of +1AP. Heavy II adds +1xSTR xDM, costs +2AP+1STA, and gives a -2 penalty to hit. Heavy III adds +1.5 x STR xDM, costs +3AP+1STA, and gives a -3 penalty to hit. Bonus applies to blunt damage and cutting damage.

\textbf{Sweeping Attack:} spends 1 additional AP on the attack and hits everything in a semicircle. Decide whether to attack from right to left or vice versa and attack the enemies in that order in a 180-degree arc. If the attack is blocked or intercepted, the impact is reduced for all. If one character is behind another, only the closest is hit. Sweeping attacks do not hit fallen enemies. 

\textbf{Opportunity Attack:} A melee weapon attack that can be used as a reaction when an opponent attempts to perform a triggering action. It is possible to cancel the triggering action and reuse the AP spent to defend against an opportunity attack. The attack requires the normal AP cost and the defense takes -2 penalty unless it's the SD. Triggering actions include anything that costs 3 AP or more during a focus surge, standing up in melee range, and moving towards a melee weapon while within its attack range. The attack occurs before the effect of the triggering action, unless stated otherwise.

\textbf{Braced Attack:} A melee attack with a weapon with the braced property that attempts to increase strength by firmly bracing the weapon. It can be used as a reaction when a target is moving towards the weapon or as an action in situations where the attacker is mounted and running. The attack happens when movement is between two spaces within weapon range. It costs an additional +2AP and +1STA on top of the normal attack cost and has damage bonus of +1.5x STR x DM on a hit. The additional damage effect also triggers a trample. Braced attack can only trigger once per turn (anyone's turn. It can be used multiple times per round.)

\textbf{Hook Attack:} A melee attack using a weapon with hook property that attempts to trigger tripping the opponent. This can be used as an action or as a reaction against running targets that move away from the weapon within two spaces, which are both inside its melee range. On a hit, the character can spend +1 AP +1STA to attempt to trip their target, in which case the attack gets +STR/2 x DM damage if the opponent attempts to perform an evasive jump and +STR x DM if they are running.

\textbf{ Assassinate :} Can only be done against SD, not against active defense, and requires a short range and precise weapon attack. This costs 1 extra AP on the normal cost of the attack. It automatically applies bypass.
}
...
\textbf{Evade:} The defender spends 2 AP to try to get out of the way of an attack. On a graze, the attack has 50\% damage, and on misses, it has 0. This can be used to avoid being trampled and it always works automatically unless trample is a directed attack. 

\textbf{Evasive Jump:} is a modification on evade. It costs an extra +1 AP +1 STA and allows the character to jump away from the attack while evading it. This movement cannot be followed and gives +AGI/2 on the skill test. This can only be used if there is space to jump. The jump must place the character further from the source of the attack and uses the movement speed of jumping backwards. When running, evasive jump uses the forward running long jump at a 60 degree angle of the direction of the run, and must end the movement further from the source of attack than it started. 

\textbf{Block:} attempts to block an attack by spending 2 AP and using any weapon attack or shield that contains the DEF property. Blocking with a shield adds its cover to defend. On graze, the attack damage is reduced by the block value. On a miss, by 1.5x as much. A blocking object can break if damage is greater than RES. 

\textbf{Intercept:} uses 3 AP to intercept the blow. It requires the defender to be in short range and can be done with anything that has the DEF property. On a graze, stops attacks unless the attacker has 5 force higher than the defender. On a miss, stop it if the attacker has less than 8 force more than the defender.
...
\skill{Force}{STR-10}{This is the skill for pushing, lifting objects, and resisting being thrown.

\textbf{Trample:} Happens when two characters hit each other at speed. This is a Force vs Force comparison. If the runner's Force is higher, the opponent moves back one space and is stunned. If Force is 5 points higher or more, the opponent is stunned and prone. Whoever is running or jumping gets a bonus equal to their running or jumping speed to this. If the defender's force is equal or higher, the runner is stopped and stunned. If a strike started this action, targeting the head increases attacker's force by 3. Trampling a prone character is an automatic success and allows free passage.

\textbf{Catch:} happens when someone tries to initiate a grapple against a running target. The catcher must spend 3 AP + 1STA to perform a strike with a weapon with the grapple property and add their own running speed to defend the trample. If the target is stopped, the catcher can decide to grapple them without further tests.

\textbf{Push and drag} attempts to move with a grappled creature. This costs 2AP +1STA and is a force vs force comparison. The strongest pushes the other by up to 1m. On a force difference equal or greater than 5, push them 2m. The defender must spend 2AP+1STA or receive -5 in this comparison. Nobody moves on a draw.

\textbf{Trip:} happens as a consequence of a hook attack. This is a comparison between the attacker's force and target's balance + force. If anyone is jumping or running, the attacker gets a bonus equal to the moving party's movement speed. If the attacker's value is higher, the target falls and is prone. Targeting the head or legs increases the attacker's value by +5.
}
...
\textbf{Immobile Targets and Shooting Errors:} If a projectile misses on the test, it hits some adjacent space or a distance even greater for every 3 points the test is below the DL.
...
\textbf{Throw:} Basic ranged attack with a throwable weapon. Throwable weapons leave the inventory once thrown.

\textbf{Shoot:} Basic ranged attack with a shooting weapon. Shoot is only allowed against targets within 30m.

\textbf{Snipe:} Modifies Shoot to cost an additional 2 AP, but allows shooting targets distance up to weapon range.

\textbf{Quick Shot:} Modifies Shoot to cost 1 AP less, but can only be used within 10m of the target. 

\textbf{Ranged attacks outside the combat area:} Any ranged attacks with range higher than 50m can hit outside the combat area at a penalty of -5.
}

\textbf{\large Explosions:} This type of attack results from something like a bomb, which radiates from a central point and loses power with distance. Explosions are defended against with a reflex test. If the explosion comes from a projectile, the DL of the explosion is equal to the shooting skill. If the explosion occurs before being perceived, no test can be made. If the bomb is perceived, players receive a certain amount of AP to move away before it occurs.
...
The damage from explosions is 150\% on a critical, 100\% on a hit, 50\% on a graze, and 0 on a miss. Effects of an explosion that have a DL, such as pushing enemies, receive +5 against enemies in the critical area and -5 against those in the graze area.
...
\textbf{Guard:} character spends 2 AP to block ranged attacks against themselves or adjacent characters, as long as they are closer to the projectile source than the adjacent character. Shield Cover is added to guard as a bonus. On graze, the attack damage is reduced by the block value of the piece of gear used. On a miss, by 1.5x as much. Hits and crits damage the character directly.

\textbf{Cover:} When using a body shield, it is possible to take total cover behind the shield after using guard. The cover is canceled if the character performs any action other than careful movement. This cover only extends to allies who stand in the same space.

\textbf{Avoiding an Explosion:} Spend 3 AP and make a reflex skill test against the DL of the explosion. On a critical, the character can run by spending one extra STA. On a hit, they can spend an extra STA to jump in any direction before the explosion occurs. On a graze, they can move 1 AP before the explosion. On a miss, they can move 2 AP after the explosion. It is possible to downgrade the movement action.

\textbf{Explosion Block:} Can only be done with a shield. Make an active reflex + shield Cover test against the DL of the explosion. If successful or critical, the explosion damage gets reduced by the defenses from the shield. The impact from explosions is unblockable. For explosions that deal cutting damage, the damage is not doubled and a hit blocks half of the damage instances and crits block all of them.
}
...
\textbf{Flanking:} when more than one character is within melee range, any attack against one of the surrounding characters triggers an attack of opportunity from the others, provided that they cannot be fit within a semi-circle centered on the triggering attack's target. Each opportunity attack is resolved in order defined by the flankers and can be voided if the target gets out of range.
...
\textbf{Pushing, catching, trampling and dragging:} work just like explained in force, but against an object DL.

The push DL depends on weight. When stopping an object, also consider its speed. To help decide on a DL, consider that 50kg (size 3) is the base dificulty, with DL 0. Change the DL by 5 for every size category. Increase the DL for higher speeds. 
...
\textbf{Drawing items in combat:} taking items from container slots takes 4 AP + a standard action. Small and Medium items in quick slots can be held in hand within a standard action during combat, large ones take two standard actions. Weapons with the draw property and all small items can be drawn with 0 AP cost as long as they are in a quick slot. 

\textbf{Putting items away:} Dropping items on the floor costs 0 AP. Storing small items and those with draw property into quick slots costs a standard action. Storing any other items costs 4 AP+ standard action.  
...
\textbf{Theft:} A standard action can be used to steal an item from someone's body or place an item on them. This is done by having one hand free and using a standard action to perform a Prestidigitation skill test against the opponent's Detection +5x for every item bulk category over small (relative to who must detect it). On criticals, it is stolen without being noticed. On successes, the character can choose between successfully stealing or failing without drawing attention. On grazes and failures, attention is drawn and it fails. Stealing an item from stealth gives a bonus of +5.

\textbf{Pick Lock:} These tests use knowledge of mechanics as enabler. On criticals, it takes 5 AP. On hits, it takes 3 rounds. On a graze, it takes 1 ET, and on a failure, it is impossible to open.
...
\begin{proplist}
    ...
    \item \textbf{Blinding Gas:} Staying inside it for any amount of time calls for a health test to avoid blindness. Protective gear over the eyes also prevents blindness.
...
\textbf{Wind:} Requires balance test to avoid being pushed, but having high enough force can automatically succeed in the test. Clears the other gases for the round. If the gas has an active source, it comes back once the wind stops.

\subsection{Liquids}

\textbf{Liquid Spread and splash:} Smalls amounts of liquid (under 50 L) are considered to spread instantly and do so with the geometry of an explosion. Liquids spread around solids, not through them. Splashing a bucket of a liquid counts as a thrown ranged attack with 3m x RM range.  
...
\textbf{Waves and Currents:} Requires Force test to avoid being pushed. If the character is swimming, they get pushed automatically, but can spend AP to swim and cancel the movement.
...
\textbf{Traps:} Require detection tests to be spotted before they fire. 
...
\textbf{Escape:} The chase starts when everyone in a party decides to run away in an open map. It always starts at the beginning of a new round. Any character that cannot run for whatever reason at the beginning of the round will prevent the chase from beginning, unless they get left behind. If they do, finish the fight, and then start the chase. 
...
\begin{proplist}
    ...
    \item \textbf{Hiding:} If the distance is not combat and there is a stealth event, in which the quarry succeeds, it escapes.
...
\textbf{Running at combat range:} This is a dash. Both sides use their movement surge to sprint, spending STA to do it. Whoever has the highest AGI x running speed wins. A draw does not change distance tier, but spends STA from the movement surge.
...
\begin{proplist}
    ...
    \item \textbf{Carrying burden:} -2 to Stealth vs hearing per burden penalty.
...
\textbf{Staying in stealth:} The character must begin their turn undetectable through any active sense in order to be considered in stealth. They must go back to being undetectable to any enemy active senses and end their turn to continue in stealth. Failing to do so causes automatic detection by everyone (their stealth value disappears).
...
\textbf{Combat morale test:} characters make a will test against the DL of the terror of the moment. A graze leaves the character afraid, and a miss activates the character's limit stress action. The limit stress action only triggers if there is an enemy within 30m. Morale tests can be done normally, safely or riskily, as the player decides.
...
\textbf{Morale test triggers:} at the beginning of a round when aggravating factors are at 5 or more, when under specific afflictions, or when specific social actions are used.

\subsection{Social actions}

Every social action costs 5 AP and counts as a Rest action, which can be done at the end of the round, just before the morale tests are triggered. They only work on creatures capable of communication.

\textbf{Intimidation:} A character can make an intimidation attempt in combat. This triggers a morale test for all enemies at the beginning of next round, adding $1 + character's charisma/2$ as aggravating factor.

\textbf{Taunt:} In combat, a taunt triggers a morale test for one enemy, adding $1 + character's charisma/2$ as aggravating factor and causing enraged instead of afraid.

Multiple intimidations and taunts don't stack. Only the highest one counts.

\textbf{Rally:} Say something encouraging in combat. This decreases the DL of allies next morale test by $1 + character's charisma/2$. Multiple Rallies don't stack. Only the highest one counts.
