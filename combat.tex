...
\textbf{Corrosive:} Its respective defense is INS. Tiers 0 to I of damage leaves the target corroding at that tier of damage, which reaplies the damage until armor is removed. 
...
\begin{proplist}
    ...
    \item \textbf{Bleed:} For every STA spent and at the end of every round in combat, the character receives +1 to their IL per bleed intensity. Bleeds caused by a T2 wound have intensity 1. For every extra tier, intensity increases by 1. Bleeding is caused by damage to the chest or head. When combat/chase ends, bleeding causes 3x its current value and stops by itself when untreated.
    \item \textbf{Shocked:} cause immobile. Happens with damage to the chest. Has 50\% chance on a T4 hit.
...
\textbf{Success Overflow:} When a character receives a hit from an attack, they may receive additional effects, chosen by the attacker. SOPs are used to activate these effects. The effects are:

\begin{proplist}
    \item \textbf{Armor Bypass:} This costs a number of SOP defined by the targets armor deflection. It hits the inner layer of the armor, if the armor has one. Only attacks from weapons with the precise property can use this effect.
    \item \textbf{Penetrating:} Costs a number of SOPs equal to the target's deflection to be able to use cutting damage against objects with the same hardness. Behaves like extra cut against fiber armor and after already penetrating same hardness objects. This can only be done by weapons with the penetrating property. 
    \item \textbf{ Extra cut:} Increases cutting damage by 1xDM per SOP. Only weapons with the bladed property can use this. Increases by 2xDM per SOP for bladed and piercing weapons.
    \item \textbf{Smash:} Costs a number of SOPs equal to the target's deflection to stun them if the damage is at least over T1.
\end{proplist}
...
\begin{proplist}
    \item \textbf{Chest:} Costs 0 points. Standard location. 
    \item \textbf{Hand:} Costs 10 points, or only 5 when used against a target that blocks with a weapon. Hands have no armor unless the character is wearing gauntlets. A wound renders the hand useless. Maximum IL damage accounted from this is 5. Taking 20 IL damage amputates.
    \item \textbf{Leg:} Costs 0 points. A wound leaves the character lame. Maximum IL damage from this is 10. Taking 30 IL amputates.
    \item \textbf{Head:} Costs 5 points. Getting a wound or stun to the head causes unconsciousness. The head has a 50\% chance of a wound on tier 1 and 100\% on tier 2 when receiving blunt damage. Tier 4 damage has 50\% chance of instant death, T5 has 100\%.
\end{proplist}
