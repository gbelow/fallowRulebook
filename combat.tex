...
\textbf{Electric damage:} It is also defended by INS. It causes interruptions when a character receives a tier 1 or higher damage from electric damage, and stuns at tier 3 or higher. This damage only deals half IL damage per tier. Electric damage dealt by a weapon that causes at least Tier 0 cutting damage ignores INS.

\subsection{Additional effects:}

\textbf{Interruption:} is an effect that causes the victim to lose their action or reaction and associated costs. It causes a minimum loss of 2 AP. It happens when a tier 1+ blunt or electric damage is inflicted or when the character is pushed. 
...
\begin{proplist}
    ...
    % \item \textbf{ Hemorrhage:} Costs a number of HOPs equal to the target's deflection to increase bleeding by 1 if damage causes at least tier 1 injury. This can only be used by weapons with the vicious property. 
    \item \textbf{Smash:} Costs a number of HOPs equal to the target's deflection to stun them if the damage is at least T1.
\end{proplist}
...
\begin{proplist}
    ...
    \item \textbf{Head:} -5 to the attack test. Getting a Tier 3 injury from blunt, cutting or electrical damage or stun to the head causes unconsciousness that lasts at least 1 minute without intervention. Tier 4 damage causes instant death. Armor bypass at the head hits flesh, which ignores all armor.
\end{proplist}
...
Shocked & T4 blunt+smash & 5 & immobile\\
\end{stattable}
...
\textbf{Wind:} Requires balance or Force test to avoid being pushed. Clears the other gases for the round. If the gas has an active source, it comes back once the wind stops.
