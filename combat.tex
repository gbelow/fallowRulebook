...
When falling, the DL to fall properly is arbitrary, but is generally higher for low falls from bad positions, average for very high falls, and lower for small falls from a controlled position. DLs should be around 20 at most and 5 at least.
...
\textbf{Combat morale test:} At the beginning of each round, all characters make a will test against the terror of the moment. A graze leaves the character frightened, and a failure activates the character's limit stress action.
...
\begin{itemize}
    \item Broken arm: Makes the arm useless. Equivalent to 10 IL to heal.
    \item Broken leg: Makes the leg lame. Equivalent to 15 IL to heal.
    ...
    \item Unconscious: causes unconsciousness. Happens when receiving damage in the head. Takes at least 1 minute to wake up without intervention.
\end{itemize}
...
\textbf{Catching:} The quarry is caught when its does not have STA to run, when it voluntarily turns back to fight or when it ends its turn with any mobility affliction while in very close range. In this case, the chase ends and a combat or social situation starts.
...
\textbf{Shot and run:} Whichever side wins can get 6 AP to perform ranged attacks instead of changing the distance when winning in the chase. The quarry can be allowed to increase distance and receive 6 AP, while the pursuers receive 12 AP to attack them. If the starting distance is combat, the distance penalty for ranged attacks is at 30m mark.
