...
\skill{Climbing}{AGI - 10}{ Make a climbing skill check to overcome a DL and climb a surface.
...
A character who is climbing is considered prone. An attack that interrupts the climber will require them to redo their climbing check with a -3 penalty. In the case of multiple climbing checks in a single turn, always take the worst result. Climbing with one hand incurs a -3 penalty.
...
\begin{proplist}
    \item \textbf{Armor Bypass:} This costs a number of HOP equal to the target's armor deflection and can only be done against rigid armor. It adds the value of 50\% RES or 50\% prot of the target armor to the damage, depending if the attack damage is blunt or cutting. Only attacks from weapons with the precise property can use this effect.
    \item \textbf{Penetrating:} Costs a number of HOPs equal to the target's deflection to be able to use cutting damage against objects with the same hardness. This can only be done by weapons with the penetrating property. 
    \item \textbf{ Extra cut:} Increases cutting damage by 1xDM per HOP. Only weapons with the bladed property can use this.
...
\begin{proplist}
    ...
    \item \textbf{Head:} -5 to the attack test. Getting stunned in the head causes unconsciousness that lasts at least 1 minute without intervention. Tier 4 damage causes instant death. Armor bypass at the head hits flesh, which ignores all armor.
\end{proplist}
...
\textbf{ Assassinate :} Can only be done against SD, not against active defense, and requires a short range and precise weapon attack. This costs 1 extra AP on the normal cost of the attack. It automatically applies armor bypass.
}
...
\textbf{Evasive Jump:} is a modification on evade. It costs an extra 1 STA and allows the character to jump away from the attack while evading it. This movement cannot be followed and gives +AGI/3 on the skill test. This can only be used if there is space to jump. The jump must move away or sideways from the attack and uses the movement speed of jumping backwards. This can be used to avoid being trampled and it always works unless trample is a directed attack.
