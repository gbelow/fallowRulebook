...
\textbf{Suffocation:} If the character cannot breathe at the beginning of a round, they lose 1 STA and cannot rest. If STA goes negative, they drop unconscious. If it goes under -10, they die.
...
When falling, the DL to fall properly is arbitrary, but is generally higher for low falls from bad positions, average for very high falls, and lower for small falls from a controlled position.

\textbf{Fall damage} is a special case of damage from impact with objects, further down this chapter. For this damage estimation, the landing spot is a flat, hard surface. The impactor is the body, so the DM is the character's DM and the damage ignores armor. The speed of the fall can be considered instantaneous for falls below 2m, in which case it causes 5xDM of blunt damage. 5m/s for the first 10m, causing 10xDM of blunt damage. Then it increases to 15m/s up to 30m, causing 20xDM of damage. Anything above that causes 40xDM of damage. A critical on the balance check reduces the damage by 50\%, a success by 25\%, a graze by nothing, and a miss hits the head. Crits and hits that don't receive T2 or higher injuries allow the character to stay on their feet, otherwise, they are prone.
}  
...
\item \textbf{Unconscious:} The character cannot do anything other than Will and Health checks. They can also make detection and sensing checks, but receive -10 on these checks. These checks can be made to wake up. The DL and the allowed check depend on the situation. Unconscious characters breathe automatically and cannot hold their breath. 
...
\textbf{Heavy Attack:} It is possible to make a heavy attack with weapons that have the "heavy" property. Heavy I increases damage of physical attacks by +0.5xSTR, but have an added cost of +1AP. Heavy II adds +1xSTR, costs +2AP+1STA, and gives a -2 penalty to hit. Heavy III adds +1.5xSTR, costs +3AP+1STA, and gives a -3 penalty to hit.
...
\begin{tabular}{||c c c||}
\hline
Fuel & Burn/round & Spread\\ [0.5ex]
\hline\hline
Dry grass, thatch & 5 & 1/round\\
Oil film & 8 & 3/round \\
Pitch, tar, resin & 10 & sticks \\
Wood pyre, timber & 10 & 0 \\
\hline
\end{tabular}
\end{center}

\textbf{Extinguishing:} Fires can be extinguished by muffling it, throwing water or with magic. Both muffling and throwing water require 4 AP and being in a burning space or adjacent to it. Not every fire is extinguishable through water or muffling within a 4 AP action.
...
Gas can have several effects: Poison, explosion, suffocation, reduced vision radius and blindness. It lasts until the end of combat or dispersed somehow through wind. Muffling or extinguishing the source of the gas causes it to disappear in the beginning of the next round. Doing that costs 4 AP. Relocating the source moves the area somewhere else starting next round.  

\textbf{Suffocation:} anyone that starts the round inside a suffocating gas is suffocating by default. Abilities and items may allow breathing.

\textbf{Reduced vision:} Smoke reduces vision radius of anyone inside of it to 2m by default and makes them invisible (untargetable through vision, requires a different active sense) to those outside. Standing at the edge of the area allows bad visibility to attackers, making them disoriented when targeting someone inside.

\textbf{Inhaled Poison:} Causes a poison effect to anyone who begins the round inside the gas or takes a rest action. If the poison gas is noticed, it is possible to self inflict suffocation to avoid it. The effects may vary depending on the poison.

In order to detect poison gas, use an appropriate knowledge test to recognize that it is poison and/or a detection test to notice its smell if the smell is weak.

\textbf{Blindness:} Being inside at the beginning of the round causes blindness. Depending on the gas, a health test may be called. Protective gear over the eyes also prevents blindness.

\textbf{Explosion:} Some gases explode in contact with fire or electricity. The damage depends on the gas and uses the rules for explosions.
...
\textbf{Liquid Spread:} Smalls amounts of liquid (under 50 L) are considered to spread instantly and do so with the geometry of an explosion. Liquids spread around solids, not through them.
...
\textbf{Traps:} Require detection tests. Gets Architecture or Survival knowledge bonus depending on where the trap is. Magical traps may use the appropriate magical knowledge.

\textbf{Collapse:} Works like an explosion, but there is no crit area. Gets buried in the rubble unless the explosion grazes or misses. Escaping requires a Strength test.
