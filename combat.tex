...
\begin{itemize}
    ...
    \item Combat surge: 3 STA to gain 6 AP that can be used for attacks or movement and must be used immediately.
    \item Reaction surge: 3 STA to gain 6 AP, but can only use reactions and rest until the end of the round.
    \item Focus surge: 1 STA to gain 2 AP. The AP is free to use for anything. This surge is required to use shooting weapons and casting spells.
\end{itemize}
...
\textbf{Shoot:} Basic ranged attack with a shooting weapon. Requires Focus surge to use.

\textbf{Snipe:} Modifies Shoot to cost an additional 2 AP to shoot with a -1 distance penalty instead of -2 for the first 50m.

\textbf{Quick Shot:} Modifies Shoot to cost 1 AP less, but incurs a -3 penalty in accuracy and -3 in distance instead of -2. 
}
...
There are four distance tiers: Combat, Shooting, Far and Very far.
...
One team may decide to fight to attempt to injure the other before they get closer or to snipe enemies early. There are two possible situations: Both teams shoot at each other or one team shoots and the other attempts to close in. Either way, the distance penalty used is that of 50m. Everyone is targetable by anyone, except when one whole team is invisible to the other.
