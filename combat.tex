...
\textbf{Stealth events:} are a description given by the player of what exactly the character is trying to do with their stealth, which defines which senses the detectors are using and sets a bonus or penalty to the detection test. The usual values should stay between -15 and +5.
...
Each event adds a bonus or penalty to the stealth value relative to a detector, who uses the best sense available. On hits or crits, the character stays unnoticed. On grazes, the character can be reacted against and raises the alarm for the detector, which allows them to go looking around actively. On misses, the character is detected and, if in combat, the opponent may take their turn immediately. The stealth value is modified by events that change the situational bonus.

Call for a new stealth check when the scene changes. One failed test means that stealth is over, so repeating multiple tests will eventually lead to failure. This is why only one test is called for a scene. The variation comes from the player's actions.

\textbf{Stealth in combat:} the stealth value is rolled in the beginning of the round. The stealthy character's actions may change the bonus of their stealth event, like when they attack someone or run. The stealth value deteriorates with the stealth event that had the worst penalty of all until the stealthy character starts a round undetectable again, at which point they remake their stealth test. Enemies cannot target the character unless their detection is higher than the stealth value and cannot react before it is 4 points below it or less.
...
\textbf{Detected and Undetected status} To be considered undetected, a character must be out of the field of detection of all active senses of another character at the beginning of a round. An undetected character cannot be reacted against.
...
\begin{itemize}
    \setlength\itemsep{0em}
    \setlength{\parskip}{0em}
    \item \textbf{Wearing camouflage:} Allows hiding in plain sight.
    \item \textbf{Hiding behind a tree:} +2 to Stealth vs. vision (Requires active search to detect).
    \item \textbf{Complete hiding:} Automatically undetected to vision.
    \item \textbf{Being in plain sight:} -10 to Stealth vs. vision. Only in front of someone.
    ...
    \item \textbf{Hitting at distance:} -2 vs Vision. Automatically detected if in plain sight. Direction is detected either way.
\end{itemize}
